"""Reads and writes for a song.

Every write here follows the same shape: change the row, stamp it with the
next rev from the setlist counter, snapshot it, commit. That order is the
invariant the sync layer and the merge both stand on — see setlists.next_rev
and revisions.record for why each half matters.
"""
...
from .schemas import SongCreate, SongDetail, SongLine, SongSummary, SongUpdate
from .setlists import next_rev


def to_summary(song: Song, setlist: Setlist) -> SongSummary:
    return SongSummary(
        id=song.id,
...
def to_detail(song: Song, setlist: Setlist) -> SongDetail:
    """Build a detail payload from an already-loaded row, so a route that
    holds a `Song` does not have to look it up a second time."""

    lines = markdown.markdown_to_lines(song.markdown_body)
    return SongDetail(
        id=song.id,
        setlist_slug=setlist.slug,
        title=song.title,
...
def list_songs(session: Session, setlist: Setlist, *, deleted: bool = False) -> List[SongSummary]:
    ...
    return [to_summary(song, setlist) for song in session.exec(query).all()]
...
def create_song(
    ...
    lines = payload.lines or markdown.default_lines()
    if isinstance(lines, list):
        validated_lines = [SongLine.model_validate(line) for line in lines]
    else:
        validated_lines = markdown.default_lines()
    markdown_body = markdown.lines_to_markdown(validated_lines)
    now = datetime.utcnow()
    song = Song(
...
def apply_metadata(song: Song, payload: SongUpdate) -> None:
    """Copy the metadata fields the payload actually set onto the row.

    `model_fields_set` rather than a None check: clearing the BPM and not
    mentioning it are different requests, and only the first should erase it.
    """
...
def update_song(
    ...
    *,
    ...
    if "lines" in payload.model_fields_set and payload.lines is not None:
        ...
        song.markdown_body = markdown.lines_to_markdown(validated_lines)
...
def purge_deleted(session: Session, setlist: Setlist, *, older_than_days: int = 30) -> int:
    ...
    for song in songs:
        revisions.drop_for_song(session, song.id)
        session.delete(song)
